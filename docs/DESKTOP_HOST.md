...
- 路径先由后端解析,**宿主不重新解析、不展开、不猜测路径**,只做本机侧的收敛
  (规范化 + 只接受配置的下载根)与存在性判断,细节见主仓库契约 §4.7;
...
第四个调用方是**下载完成通知**(`useDownloadCompletionNotice`,挂在下载页):
它复用同一对按钮,但只播报**真实变化**。任务列表首帧是历史 —— 后端把数据库里
已完成的任务与内存中的活动任务合并后返回,所以第一次快照里全是"早就完成"的记录;
该 hook 因此先把首帧里的已完成任务记为已读,之后才播报新出现的完成项(无论是
看着它从 running 变成 completed,还是在两次轮询之间跑完的)。新增任何"任务完成"
类反馈都沿用这条规则,不要用列表长度、轮询次数或"最近一条完成记录"来判断。
